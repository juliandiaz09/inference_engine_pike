...
class AvianExpertApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.title("Sistema experto de identificaciÃ³n de aves")
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Label(
            hero,
            text="Sistema experto para clasificaciÃ³n de aves",
            style="HeroTitle.TLabel",
        ).grid(row=0, column=0, sticky="w")
        ttk.Label(
            hero,
            text=(
                "Identifica especies de aves ingresando caracterÃ­sticas observadas. "
                "Puedes elegir encadenamiento hacia adelante o hacia atrÃ¡s para ver "
                "el proceso de inferencia con reglas taxonÃ³micas verificables."
            ),
    ...
    def _draw_flow(self, mode: str) -> None:
        ...
        caption = (
            "Flujo hacia adelante: hechos -> reglas -> conclusiÃ³n"
            if mode == RECOGNITION_MODE_FORWARD
            else "Flujo hacia atrÃ¡s: meta -> subobjetivos -> hechos"
        )
        canvas.create_text(
            width // 2,
            96,
