...
def perfil_coincidente(rule, context = None, index = None):
  ...
  try:
    with knowledge_base.Gen_once if index == 0 \
             else engine.lookup('animals', 'perfil_animal', context,
                                rule.foreach_patterns(0)) \
      as gen_0:
      for dummy in gen_0:
        with knowledge_base.Gen_once if index == 1 \
                 else engine.lookup('animals', 'caracteristica', context,
                    ...
                    with knowledge_base.Gen_once if index == 4 \
                      ...
                      for dummy in gen_4:
                        with knowledge_base.Gen_once if index == 5 \
                          ...
                          as gen_5:
                          for dummy in gen_5:
                            engine.assert_('animals', 'candidato',
                                           (rule.pattern(0).as_data(context),)),
                            engine.assert_('animals', 'taxonomia',
                                           (rule.pattern(0).as_data(context),
